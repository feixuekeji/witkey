...
//获取数据列数，实现翻页
$sqlCnt="select count(*) from message order by ID";
...
$sqlUser="select * from message order by ID limit {$off},{$length}";
...
<body>
	<center>
		...
		<?php
		//
		echo "<h3><form action='index.php' method='get'>共{$total}条    每页显示<input name='length' type='text' value=$length size='5'> 条 <input type='submit' value='确认'></form>   </h3> 
